...
from call_api import (
    call_text_endpoint,
    call_photo_endpoint,
    call_hybrid_endpoint,
    call_greetings_endpoint,
    call_recommender_api,
)
...
def textq_search():
    ...
    with st.form("text_search_form"):
        ...
        k = st.slider(
            "Number of Images to retrieve", min_value=1, max_value=20, value=6
        )
        submitted = st.form_submit_button("Search")
        if submitted:
            reformulated_queries = asyncio.run(call_recommender_api(text_query))
...
def imageq_search():
    ...
    with st.form("my_form"):
        ...
        with col1:
            image = load_image(ROOT_DIR + get_random_image_file("./cached_images"))
...
def hybridq_search():
    ...
    with st.form("my_form"):
        ...
        with col3:
            k = st.slider(
                "Number of Images to retrieve", min_value=1, max_value=3, value=3
            )

        with col4:
            image_weight = st.slider(
                "Photo Weight", min_value=0.0, max_value=1.0, value=0.5, step=0.01
            )
